...
function sampleMesh(scene: THREE.Group, count: number): CloudSample {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (bucket.albedo && bucket.uvs) {
      const tu =
        bucket.uvs.getX(i0) * w + bucket.uvs.getX(i1) * u + bucket.uvs.getX(i2) * v;
      const tv =
        bucket.uvs.getY(i0) * w + bucket.uvs.getY(i1) * u + bucket.uvs.getY(i2) * v;
...
export function ParticleObject({ className, style, ...options }: ParticleObjectProps) {
