...
export function createShatter(
  ...
  let contentMaxX = 1;
  let fallbackDirty = !htmlInCanvas;

  function rasterizeContentFallback() {
    if (htmlInCanvas || !sourceCtx || !fallbackDirty) return;
    const dpr = Math.min(window.devicePixelRatio || 1, 2);
    const cssW = Math.max(1, content.clientWidth);
    const cssH = Math.max(1, content.clientHeight);
    ...
    sourceCtx.fillStyle = cs.color || "#111";
    sourceCtx.font = cs.font || `600 ${fontSize}px serif`;
    sourceCtx.textAlign = "left";
    sourceCtx.textBaseline = "alphabetic";
    if ("letterSpacing" in sourceCtx) {
      (sourceCtx as CanvasRenderingContext2D).letterSpacing = cs.letterSpacing;
    }
    ...
    const y = Math.max(
      fontSize * 0.82,
      targetRect.top - contentRect.top + fontSize * 0.82
    );
  ...
  function uploadContent() {
    if (!htmlInCanvas) rasterizeContentFallback();
  ...
  const themeObserver =
    !htmlInCanvas && typeof MutationObserver !== "undefined"
  ...
  if (!htmlInCanvas && document.fonts?.ready) {
...
export function Shatter({ children, className, style, ...options }: ShatterProps) {
