...
export function createGlyphRain(
  ...
  // Prefer window so fixed full-page overlays with pointer-events:none still stir.
  ...
  function onPointerMove(event: Event) {
    ...
    const x = pointerNorm(event as PointerEvent);
    if (x < 0 || x > 1) {
      tracking = false;
      return;
    }
  ...
  pointerHost.addEventListener("blur", onPointerLeave);
  ...
  return {
    ...
    destroy() {
      ...
      pointerHost.removeEventListener("blur", onPointerLeave);
...
export function GlyphRain({ children, className, style, ...options }: GlyphRainProps) {
  ...
  useEffect(() => {
    ...
    instanceRef.current = createGlyphRain({ source, content, output }, initialOptions);
