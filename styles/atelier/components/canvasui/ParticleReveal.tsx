    ...
    setOptions(next) {
      if (
        !Object.entries(next).some(
          ([key, value]) => config[key as keyof ParticleRevealOptions] !== value
        )
      )
        return;
...
export function ParticleReveal({
  ...
  return (
    <div className={className} style={{ position: "relative", ...style }}>
      {/* In-flow shell establishes height so absolute canvases don't collapse + scroll */}
      <div
        ref={!native ? contentRef : undefined}
        aria-hidden={native || undefined}
        style={{
          position: "relative",
          width: "100%",
          overflow: "hidden",
          visibility: native ? "hidden" : "visible",
          pointerEvents: native ? "none" : "auto",
        }}
      >
        {children}
      </div>
